...
The smallest runnable slice of this loop — one turn, the single adaptation type, and the visible "Mirror noticed…" reflection beat — is specified in [`docs/CORE_LOOP.md`](docs/CORE_LOOP.md) and operationalized in [`loop/`](loop/). Run the fully worked example with `python -m loop`. The companion **feel-spec** for one beat — the ~30-second player envelope, tone signature, and the feel-breakers M1 rejects — lives in [`docs/core_loop_feel.md`](docs/core_loop_feel.md); CORE_LOOP locks structure, the feel-spec locks experience. The three M1 gameplay locks those documents serve (one mirror axis, one adaptation site, one Reflection) are recorded in [`docs/adr/0001-m1-locks.md`](docs/adr/0001-m1-locks.md).
